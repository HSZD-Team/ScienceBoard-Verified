...
class VManager(Manager):
    ...
    INIT_NAME = "sci_bench"
    SERVER_PORT = 5000
    A11Y_EMPTY_RETRIES = 5
    A11Y_EMPTY_RETRY_DELAY = 2
    ...
    def env(self, value: VirtualEnv) -> None:
        ...
        if self.key not in ENVS:
            ...
            try:
                ...
                # The bundled adapter covers the VMware API surface used here.
                from .desktop_env_fallback import DesktopEnv
    ...
    def a11y_tree(self) -> Optional[str]:
        raw_a11y_tree = None
        for attempt in range(VManager.A11Y_EMPTY_RETRIES):
            raw_a11y_tree = self.controller.get_accessibility_tree()
            if isinstance(raw_a11y_tree, str) and raw_a11y_tree.strip():
                break
            if attempt + 1 < VManager.A11Y_EMPTY_RETRIES:
                self.vlog.warning(
                    "Received an empty accessibility tree; retrying observation."
                )
                time.sleep(VManager.A11Y_EMPTY_RETRY_DELAY)

        assert isinstance(raw_a11y_tree, str) and raw_a11y_tree.strip()
