...
class Primitive:
    ...
    @option_handler
    def ANS(*args) -> None:
        """When you are asked to submit an answer, replace s with the exact answer and return only the following code block:
«
ANS s
»
Do not return ANS s as plain text, do not put it on the same line as the code fences, and do not add text outside the code block. Use «FAIL» if there is no answer to the question"""
