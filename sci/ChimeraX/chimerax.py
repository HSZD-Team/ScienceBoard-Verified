...
class VMManager(VManager, ManagerMixin):
    # ChimeraX can expose its VM proxy endpoint before the remote-control
    # extension has finished binding its REST port.  Keep the task's original
    # launch command and wait, then retry only an empty/non-JSON proxy reply.
    REST_READY_RETRIES = 15
    REST_READY_INTERVAL = 2
    ...
    def _execute(self, command: str) -> Dict:
        ...
        for attempt in range(1, self.REST_READY_RETRIES + 1):
            try:
                ...
                last_error = error
                if attempt == self.REST_READY_RETRIES:
                    raise
                self.vlog.info(
                    "ChimeraX REST endpoint is not ready; "
                    f"retrying ({attempt}/{self.REST_READY_RETRIES - 1})."
                )
                Manager.pause(self.REST_READY_INTERVAL)
